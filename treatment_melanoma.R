...
within_stage_shift_early<-function(policies, withinshifts, withinstages,withinHR1){
  wH1<-data.frame(wh1=c(withinHR1,1))
  wH2<-data.frame(wh2=c(1,1,0,0))
  wHRes<-withinshifts
  wHR1<-withinshifts
  wHR2<-withinstages
  mortratesw1<-withinshifts
  mortratesw2<-withinshifts
  policy1 <-policies$num[which(policies$earlydetHR==1)]
    for (x in policy1) {
    wHRes[policy1][[x]][,]<-1
    }
   policy2<-policies$num[which(policies$earlydetHR!=1)]  
     for (x in policy2) {
       wHR1[[x]]<-wHR1[[x]]+1
       mortratesw1[x] <- lapply(wHR1[x], return_value_from_id, df=wH1, value='wh1')
       mortratesw2[x] <- lapply(wHR2[x], return_value_from_id, df=wH2, value='wh2')
       wHRes[x] <- lapply(x,
                            FUN=function(x, HR, rate) {
                              HR[[x]]*rate[[x]]
                            },
                            HR=mortratesw1, rate= mortratesw2)
       
     }
   v1 <- unlist(wHRes)
   wHRes<-relist(replace(v1, v1==0, 1), skeleton=wHRes)
return(wHRes)
}

within_stage_shift_advanced<-function(policies, withinshifts, withinstages,withinHR2){
  wH1<-data.frame(wh1=c(withinHR2,1))
  wH2<-data.frame(wh2=c(0,0,1,1))
  wHRas<-withinshifts
  wHR1<-withinshifts
  wHR2<-withinstages
  mortratesw1<-withinshifts
  ...
  v1 <- unlist(wHRas)
  wHRas<-relist(replace(v1, v1==0, 1), skeleton=wHRas)
  return(wHRas)
} 
#-------------------------------------------------------------------------------
