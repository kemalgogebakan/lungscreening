...
#' 
#' @export
simpolicies_mel <- function(scenarios, naturalhist, treatinfo, overd1,overd2,ppv,advanced1,advanced2,withinstageshift,withinHR1,withinHR2,
  ...
  #Within stage shift mortality benefit
  
    
  # Baseline mortality
  mortrates <- lapply(newstages, return_value_from_id, df=naturalhist,value='mortrate')
  ...
  finalmortrates1 <- lapply(scenarios$num, 
                           FUN=function(x, HR, rate) {
                             HR[[x]]*rate[[x]]
                           },
                           HR=HRs, rate=mortrates)
  
  if (withinstageshift==1) {
    withinmortrates1<-within_stage_shift_early(policies=scenarios, withinshifts=stageshifts, withinstages=newstages, withinHR1=withinHR1)
    withinmortrates2<-within_stage_shift_advanced(policies=scenarios, withinshifts=stageshifts, withinstages=newstages, withinHR2=withinHR2)
    finalmortrates2<-lapply(scenarios$num, 
                          FUN=function(x, HR1, rate) {
                            HR1[[x]]*rate[[x]]
                          },
                          HR1=withinmortrates1,rate=finalmortrates1)
    finalmortrates<-lapply(scenarios$num, 
                          FUN=function(x, HR2, rate) {
                           HR2[[x]]*rate[[x]]
                          },
                         HR2=withinmortrates2,rate=finalmortrates2)
  } else {
  finalmortrates<-finalmortrates1
  }
  ...
  nns[[4]]<- 20000/arr[[4]]
  nns[[5]]<- 20000/(arr[[5]]-arr[[2]])
  nns[[6]]<- 20000/(arr[[6]]-arr[[3]]) 
  livess<-cum_inc
  mr<-cum_inc
   for (i in 1:3)
